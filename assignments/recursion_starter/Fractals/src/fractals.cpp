...
void drawSierpinskiTriangle(GWindow& gw, double x, double y, double size, int order) {
    ...
    if (order == 0) {return;}
    if (order == 1) {
        double x_right = x + size;
        double y_right = y;
        double x_down = x + size/2;
        double y_down = y + sqrt(3)*size/2;
        gw.drawLine(x, y, x_right, y_right);
        gw.drawLine(x, y, x_down, y_down);
        gw.drawLine(x_right, y_right, x_down, y_down);
        return;
    }
...
void mandelbrotSet(GWindow& gw, double minX, double incX,
    ...
    for (int i = 0; minX + i * incX < width; i++) {
        for (int j = 0; minY + j * incY < height; j++) {
            int used_iter = mandelbrotSetIterations(Complex( minX + i * incX, minY + j * incY), maxIterations);
            cout << "i, j, used_iter: " << i << " " << j << " " << used_iter;
            if (color == 0) {
                pixels[i][j] = palette[used_iter % palette.size()];
            } else {
                if (used_iter == maxIterations) {
                    pixels[i][j] = color;
                }
            }
        }
...
int mandelbrotSetIterations(Complex z, Complex c, int remainingIterations) {
    // TODO: write this function
    if (remainingIterations == 0 || z.abs() > 4) {return 0;}
    Complex z_next = z * z + c;
    return 1 + mandelbrotSetIterations(z_next, c, remainingIterations-1);
}
